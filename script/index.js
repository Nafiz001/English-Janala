...
const loadLevelWord=(id)=>{
    manageSpinner(true);
    fetch(`https://openapi.programming-hero.com/api/level/${id}`).then(res => res.json()).then(data=>
        {
            removeActive();
...
const displayLevelWord=(words)=>{
    ...
    if (words.length == 0){
        ...
        <img src="../assets/alert-error.png" class="mx-auto ">
            <p class="text-xl font-medium text-gray-400 space-y-6 rounded py-5">এই Lesson এ এখনো কোন Vocabulary যুক্ত করা হয়নি।</p>
    ...
    words.forEach((word) => {
        ...
        card.innerHTML= `<div class="bg-white rounded-xl shadow-sm text-center py-10 px-5">
            <h2 class="font-bold text-2xl">${word.word ? word.word: "Not found word"} </h2>
            <p class="font-semibold">Meanig / Pronunciation</p>
            <div class="text-2xl font-medium font-bangla">
            ${word.meaning ? word.meaning: "Not found meaning"}/${word.pronunciation ? word.pronunciation: "Not found pronunciation"}
            </div>
...
document.getElementById("btn-search").addEventListener("click",()=>{
    ...
    fetch("https://openapi.programming-hero.com/api/words/all").then(res=>res.json()).then(json=>{
